...
Date: 2026-08-10
Status: accepted
Superseded-by: ADR-0147 — retires only Part 3's "the push, the PR and an accepted security residual stay the operator's on both routes" clause, and only for the push and the PR. They hold word for word wherever an operator is at the run; under an unattended run the push of that run's own `oso-run/<change>` branch and the PR that closes it are the FINISH the plan approval chartered, while the PR MERGE, a release, a production deploy and an accepted security residual stay the operator's on every route. The machine entry condition itself, the entry-and-never-a-permission reading it carries, the chain's two words, the four exits and the set-aside that undoes nothing all stand
