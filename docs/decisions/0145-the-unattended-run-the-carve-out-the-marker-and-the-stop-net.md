...
Superseded-by: ADR-0150 — retires only Part 3's predicate, "when a turn ends without parking or closing the run", which is incomplete rather than wrong: a THIRD class of healthy turn-end exists, a turn that ended because a delegation is still running, and the flow marks it with `auto_wait` so the net holds instead of pushing. Everything else stands. Part 1's marker keeps both reader properties, and the new key takes the same discipline — session-keyed, validated where it is read. Part 2's delivery carve-out is untouched entire. The cap is still three pushes counted in journal BYTES, and a held turn spends none of it. Part 3's fail-open direction above all: that decision fixed it and ADR-0150 EXTENDS it to its own paths rather than moving it, since the hold is itself an allowed stop
