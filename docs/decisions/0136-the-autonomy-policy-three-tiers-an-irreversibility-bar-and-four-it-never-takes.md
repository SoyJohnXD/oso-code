...
Date: 2026-08-10
Status: accepted
Superseded-by: ADR-0147 — retires only Part 4's "fixed at four" count and the scope of its first item, under the growth clause this decision's own Consequences wrote. The list is FIVE: a production deploy enters explicitly, and "a push, a PR, or a release" is re-scoped to a push onto a SHARED branch, the PR MERGE and a release, since an unattended run's own change branch and the PR that closes it are the finish its approval chartered. Part 1's ordering, Part 2's three tiers, Part 3's irreversibility bar, the delegated record and the four other refusals all stand — and the second tier gains a citation duty rather than losing anything
