...
- **The rubric and plan halves match case-SENSITIVELY** (`grep -F` for the anchor, `grep -qF` for each marker), so a reword that keeps the clause in other words — or that changes a marker's capitals — fails, which errs toward flagging rather than toward missing one. The ESCALATE half rides `anchored_absolute_stays_bounded_in`, which matches case-INSENSITIVELY, so its four markers are the only ones of the rule's twelve whose capitals are not enforced. The mismatch is inside one rule, and a maintainer editing markers has to know which half they are in.
