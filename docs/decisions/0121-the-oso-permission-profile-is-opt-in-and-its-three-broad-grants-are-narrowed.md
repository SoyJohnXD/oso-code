...
Reconciled: applied — the rendered `config.toml` makes `:workspace` (Codex's own stock profile) the machine default and moves the oso profile behind an explicit per-invocation override; `.git`, network and the secret denylist are each narrowed to what was verified, empirically, to be both necessary and safe to grant; `bootstrap/verify-codex.sh` gains a second host-contract check (alongside ADR-0106's) proving the override mechanism this decision relies on is real.
