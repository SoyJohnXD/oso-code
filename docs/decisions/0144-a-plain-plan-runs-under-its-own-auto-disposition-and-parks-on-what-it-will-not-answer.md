...
Date: 2026-08-10
Status: accepted
Superseded-by: ADR-0145 — retires only Part 1's "no fourth key and no gate that reads one" clause, and by that decision's own test rather than by a change of mind: three hooks now read the marker it arms. The disposition still lives in this change's ledger and the index row's `NEXT:` line, `mode=plan` still stands throughout, and the runtime triple §5 and §6 write is untouched. The two arming routes, the flip as its own consent, phases 1–5 with the operator, the borrowed policy, the two substitutions, the park and the reporting bound's second exception all stand unchanged
