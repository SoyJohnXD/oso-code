...
Superseded-by: ADR-0126 — retires only the "Wave 1's WAVE START is the CHANGE BASE §3 recorded" clause below; that held only when no design-foundation slice ran, and this Decision's own Context wrongly generalized it to the wave-0 case. Every other coordinate this decision names — CHANGE BASE, later waves' WAVE START, SLICE START under both modes, and triage's comparison — stands unchanged.
