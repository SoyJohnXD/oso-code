...
Date: 2026-08-07
Status: accepted
Reconciled: applied — every comment citation is gone from the six `bash -n` sets `ci.yml` covers, the two human-facing diagnostics that cited a decision as their authority name the fact instead, `Implemented-in:` is gone from all sixty-three decision files that carried it when this slice ran — the sixty-one the log held at this change's base ref, plus ADR-0130 and ADR-0131, which earlier slices of this same change wrote under the convention while it was still live, so they never appear as removals in a diff taken against that ref — and out of the blueprint's Decisions preamble, which counted the citing files it held among the fields a decision file carries and now names the source line in their place, `tests/plugin-lint.sh`'s bidirectional rule is narrowed to a reference-resolves check and joined by `check_executables_carry_no_decision_citations`, and `bodies/plan.md`'s verification-bar-coupling exemplar teaches the same lesson off the rule-count relation instead. `tools/hook-gates.txt` is cleared too, past the boundary below rather than under it: it is the source both hook manifests render from and it carries a comment header, but it is data and no `bash -n` set reaches it, so the rule cannot see what review took out of it. `docs/decisions/0077`'s body still describes the retired relation as this repo's example of that edge: a decision body stays as written, so this file is where the retirement is recorded rather than there.
...
- A decision file's frontmatter is now `Date`, `Status`, an optional `Supersedes` or `Superseded-by`, `Reconciled` and `Source`. Nothing in the repo needs a decision to enumerate its implementation, so nothing asks a writer to keep that list current.
