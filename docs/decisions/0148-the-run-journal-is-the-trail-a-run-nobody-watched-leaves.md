...
Date: 2026-08-13
Status: accepted
Superseded-by: ADR-0150 — retires two clauses and nothing else. Part 3's "one that did not is a run being restarted into the same place" is broken by one class the byte signal cannot see: a run whose delegation outlived the net's 45-minute ceiling is pushed with a journal that did not grow and is not being restarted into the same place, which is why that push carries its own give-up wording rather than the "no progress" one this clause underwrites. And the Consequences' "Two files, one location, one lifetime" is THREE — the wait sidecar joins the journal and the push tally under the same directory, the same private mode and the same untouched-by-cleanup lifetime, so the pairing argument holds at three exactly as it held at two. The journal itself, its per-change keying, the three counts against the event log, the net's size-only read of it and the format sensitivity that read creates all stand. ADR-0161 narrows that size-only read one step further than ADR-0150 anticipated: the net still reads the file's size, but only to RENEW a standing wait mark, never to decide that a push made progress, so Part 3's "it reads the file's SIZE and nothing else" and the sentence making the journal the net's progress signal are retired — a run obeying the unattended contract journals every milestone, which made a working run and a restarted one read alike, and the push tally's notion of progress is the run's own state changing now. The journal itself, its per-change keying, the three counts against the event log and the format sensitivity that read creates are unaffected and still stand
