...
The matcher becomes a lexer in `plugin/hooks/lexer.sh` — sourced by `lib.sh`, which keeps the payload reading, state, verdicts and telemetry every gate shares — that distinguishes CODE from TEXT: quote, escape and subshell state; heredoc excision with the shell-interpreter asymmetry; comment stripping; command-position enumeration; prefix stripping (`env X=1 sudo -n timeout 60 git commit` runs git); a basename match on the command word, so `/usr/bin/git` and `git.exe` count; and an interpreter payload resolved by position — the first non-option word after `-c`, never the line's last argument.

The TypeScript lexer now reads repeated `env -u NAME`, `--unset NAME`, `--unset=NAME` and `--` explicitly; unsupported or malformed options and expanded executable positions remain unread. Here-strings are stdin data for literal `sha256sum` consumers and the bounded `while IFS= read -r file; do sha256sum "$file"; done <<< "$changed"` pattern; other consumers remain uncertain or undergo interpreter payload analysis, and executable substitutions are always analyzed.
