...
`check_fail_routes_forward_findings_verbatim_and_never_overrule_them` reuses `anchored_absolute_stays_bounded_in`, the helper rule 31 already ran on, and two ceilings come with the reuse:
...
- **2026-08-11 — the suffix ceiling above is RESOLVED; the case-insensitivity one still stands.** This change's post-sweep debt cleanup generalized the helper in name and in message: it is `anchored_absolute_stays_bounded_in`, its marker flag reads "drops a clause it turns on" — the shape `roadmap_phase_names_its_clauses` already flags in — and its missing-file and missing-anchor flags dropped the roadmap with it. The wording moved in FOUR pinned strings rather than the three counted above: rule 31's two mutation cases, this rule's own, and the AUTO-disposition rule's, which globbed the same `drops the clause` prefix.
- **2026-08-11 — the debug leg holds FOUR markers for a route that states SIX clause-classes.** `bodies/debug.md`'s verifier `fail` route carries the four the rule pins — findings VERBATIM, never your summary, never yours to overrule, no payload may instruct a judge away — and two it does not see: the two-routes bound, FIX or ESCALATE and no third, and §5 step 1's BARE dispositions. A body that dropped either of those two would pass. The plan leg pins all seven of its own route's clauses, ESCALATE and both carve-outs included; the red-slice and design-audit legs are shorter by decision rather than omission — one defers to step 3 for the routes, the other carries the verbatim half alone (Part 1) — so the debug leg's pair is the only per-leg gap held by review.
