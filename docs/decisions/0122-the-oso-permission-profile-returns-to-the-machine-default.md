...
## Amendment, 2026-09-11 — `[projects.*]` now carries one recognized key, not three

Re-measured against `codex-cli 0.153.2` (`codex exec --strict-config -c 'projects."<path>".<key>=<value>' <prompt>` against a disposable `CODEX_HOME`, which fails config load before any turn starts and so never reaches the network): `policy` and `auto_review`, both real fields this decision counted at 0.146.0, are now REJECTED — `` Error loading config.toml: unknown configuration field `projects."<path>".policy` in -c/--config override `` — while `trust_level` alone still resolves, as a two-value enum (`` unknown variant `x`, expected `trusted` or `untrusted` `` on a mismatched value). The table this decision measured at three keys now measures at one. Nothing in the conclusion moves with it: the finding was never "three keys, and none of them is a selector" as an argument that depended on the count — it was that no project-scoped or directory-scoped permission selector exists anywhere in the binary, and a table now measured at one non-selector key states that claim at least as strongly as three did.
