...
Date: 2026-08-05
Status: accepted
Superseded-by: ADR-0139 — retires two clauses only: the "Five milestones" enumeration, which gains a sixth at the change level that only a roadmap reaches, and "the named residual — the one exception to both rules above", which becomes two exceptions under a closed list. The required facts per milestone, the three-line bound, the one-platform-file-per-host binding and the one-definition-never-a-copy rule all still stand. ADR-0145 retires one clause more: the Delivery section's order as an ABSOLUTE, for a milestone reported by an unattended run on a host whose platform file carves one out — the required facts, the bound and the per-host binding are what govern that milestone instead, and the order itself still holds for every other delivery and on every host that carves out nothing
