...
- [0113](decisions/0113-stale-state-warning-is-scoped-to-this-repositorys-own-file.md) — The stale-state warning names only this repository's own file
...
**2026-08-07 — clean-bar-convergence**
...
**2026-08-10 — roadmap-auto-mode**

- [0135](decisions/0135-a-fourth-mode-roadmap-and-one-approval-over-the-whole-queue.md) — A fourth mode: `/roadmap`, and one approval over the whole queue
- [0136](decisions/0136-the-autonomy-policy-three-tiers-an-irreversibility-bar-and-four-it-never-takes.md) — The autonomy policy: three tiers, an irreversibility bar, and four decisions it never takes
...
**2026-08-10 — authority-plan-auto**

- [0141](decisions/0141-fail-routes-carry-findings-verbatim-and-never-overrule-a-no-exception-finding.md) — Fail routes carry a verdict's findings verbatim, and never overrule a no-exception finding
- [0142](decisions/0142-the-verifiers-payload-is-a-closed-list-and-its-comment-gate-is-empirical.md) — The verifier's payload is a closed list, and its inline-comment gate is empirical
- [0143](decisions/0143-the-four-no-exception-rules-outrank-the-repo-under-judgment.md) — The four no-exception rules outrank the conventions of the repo under judgment
- [0144](decisions/0144-a-plain-plan-runs-under-its-own-auto-disposition-and-parks-on-what-it-will-not-answer.md) — A plain plan runs under its own AUTO disposition, and parks on what the policy will not answer

**2026-08-13 — auto-continuity**

- [0145](decisions/0145-the-unattended-run-the-carve-out-the-marker-and-the-stop-net.md) — The unattended run: the delivery carve-out, the marker, and the Stop net
- [0146](decisions/0146-the-context-economy-a-bounded-window-and-a-re-anchor-floor.md) — The context economy: a bounded window the harness cannot guarantee, and a re-anchor as the floor
- [0147](decisions/0147-the-auto-ceiling-local-and-staging-the-pr-finish-production-never.md) — The AUTO ceiling: local and staging, the PR finish, production never
...
Planning runs entirely in the host's native Plan Mode (read-only, harness-enforced), entered after a resume check. A child of Mode 4 is where that reads differently, and on Claude Code alone: there the single `ExitPlanMode` approval at that mode's phase 3 is the transition out of Plan Mode as much as the approval, and the way back out of it is a second approval of the same kind — the one thing that mode's single approval was given in place of — so a child enters it nowhere, its phases 1–5 run outside native Plan Mode, and the Repaso-headed document phase 5 builds is delivered as plain text under the host's delivery contract with the roadmap's one approval covering it (ADR-0135). Read-only there is this flow's own rule — nothing before phase 6 writes code — rather than the host's. Codex asks the operator to re-enter native Plan Mode for each child, so a child's phases do run inside it there, at the cost the parity ledger's assisted-roadmap loss row records. The behavioral phases below are shared; host tools and lifecycle spellings are adapter facts under ADR-0096 and are compared in the parity ledger required by ADR-0097.

0. **Resume check** — search engram for `oso/index` (direct topic-key search as fallback when the index doesn't exist yet) to find prior work on this change; resume from the recorded phase instead of re-asking decisions already made. Read `oso/preferences` here too — ONE record per project, at the only scope `mem_search` can retrieve at (ADR-0149): on the first plan in a project (no observation yet) ask one round of two behavior questions and save the observation, migrating a legacy `scope: personal` copy found in this project in place rather than re-asking it; thereafter apply it silently. Its three CEILING fields belong to a different moment entirely — the first AUTO or roadmap arming, never here — so a project that never runs unattended is never asked for its production route.
...
3. **Decision rounds** — as many rounds as complexity demands. Each round uses the host question tool up to its platform cap — 4 questions on Claude Code, 3 on Codex — with options and tradeoffs, sourced from the surface-mapping battery. Everything lands in a **decision ledger**: contracts, architecture, data model, error handling. Exit: invariant core plus every derived category covered, an optional pre-freeze doubt pass on migrations/security/rollback triggers, + human declares the ledger frozen. The agent never assumes — an uncovered decision during execution stops the work and returns to the human. Under a roadmap (Mode 4) there is no human to return to, so the roadmap's declared policy answers in their place and the answer is recorded as delegated, or the decision is queued for that mode's presence phase and this change is set aside where it stands (ADR-0136, ADR-0137) — never assumed, on either route. A change running under its own AUTO disposition reads that way too, with exactly two substitutions: the queue is this run's own final report, and the set-aside is a PARK behind a blocked report rather than the next child's arming (ADR-0144).
4. **Slicing** — vertical slices, each with a goal, estimated files, its own verify criteria (at least one automated check that fails without the slice, or a declared `Verify-exception`), and the slices it depends on. The cut is by that vertical bar ALONE, never by the execution mode chosen at the end of this phase: the dependency graph is derived from the cut, and cutting for parallelism is what produces horizontal, individually unverifiable slices. Its edges are read off the surface map — a contract and its consumers, shared state two slices both write, a data flow running out of one into the other, and verification-bar coupling, the one none of the other three shows — with files overlap only as a secondary, physical check. A front change in a project with no `PRODUCT.md`/`DESIGN.md` opens with a design-foundation slice the orchestrator runs itself (Impeccable's `init` or `document`); a contract change with many consumers offers the expand-contract template (EXPAND → MIGRATE → CONTRACT, the CONTRACT slice gated on a named pre-delete completeness check). The graph groups into WAVES — a wave holds slices with no edge between any two of them, wave 0 is the design-foundation slice alone in the main checkout, and a CONTRACT slice never shares a wave with a MIGRATE one. Present the slices in wave order with the widest wave's width, then settle execution in ONE ROUND of two questions. The first carries that width, the arithmetic and a recommendation: run SEQUENTIALLY in the main checkout, or run each wave in PARALLEL with one worktree per slice — parallel recommended at a width of 3 or more, concurrency capped at 4 by default, and unavailable at all when the base ref is `none`. The second is the execution DISPOSITION (ADR-0144): run phases 6 and 7 NORMALLY, with the operator at every point that puts a decision to them, or under AUTO, where the policy answers in their place at this change's scale and the run parks on what that policy will not answer — NORMAL is the default and the recommendation, and the question is skipped with the recorded answer when the ledger already carries one. The mode, the cap and the disposition with the date it was taken go in the ledger; approval still happens through the Repaso-headed plan document (phase 5), which is where the single gate is, not here.
...
## Mode 4 — `/roadmap` (a queue of changes decided once)

A queue of substantial changes planned in one sitting and then carried without the operator (ADR-0135). Every child is a Mode 1 change and nothing else is queued: a quick or debug child is a recorded YAGNI rather than a silent omission, and an ask that belongs in either mode meets the answer Mode 1's own phase 1 gives it. The exchange with the operator is phases 1–3; everything after it runs on what that exchange recorded, which is the whole trade this mode offers.

1. **The queue** — three artifacts, approved together at phase 3: the CHILDREN in run order, each carrying an intent, its mode, and whether it already arrives with an approved plan; the GLOBAL LEDGER of decisions that hold across the roadmap, every entry naming the children it binds; and each child's own important decisions. A global entry is INHERITED by a child's ledger naming the entry it came from, a per-child decision OVERRIDES it for that child alone, and an inherited entry the child's own evidence contradicts is a RECONCILIATION — never resolved silently in either direction. It persists as `oso/{roadmap}/ledger` plus one `oso/index` row at status `roadmap`, whose `NEXT:` line is where the roadmap's position lives.
2. **The autonomy policy** (ADR-0136) — declared BEFORE the approval, so what answers a decision in the operator's absence is something they agreed to rather than something the flow improvised. Two outcomes and no third: a decision it resolves is taken and recorded as DELEGATED naming the tier that answered, and one that structurally needs the human is QUEUED for phase 5 while the chain moves on. Three tiers, each reached only when the one above it has nothing to say: the flow's own recommendation put the way its rounds are already required to put one, then what current practice standardizes, then what costs the operator least to live with. Over the IRREVERSIBILITY BAR — an undo needing more than an edit and a re-run in this repository — it defers instead of deciding, and FIVE things are never a tier's question at all (ADR-0147, under the growth clause ADR-0136 wrote for its own list): a push onto a SHARED branch, the PR MERGE or a release — an unattended run's own change branch and the PR that closes it being the finish its approval chartered rather than a refusal; a production deploy; a ledger amendment; an accepted security residual; a forced deletion.
3. **Approval — once, for the whole roadmap** — one gate covering the planning behind the queue and the execution of every child in it. What it does not buy: a queue that changed materially after presentation (re-present and pass the gate again), a child's own plan document (every child still builds and delivers Mode 1's Repaso-headed one), and a decision the policy cannot resolve. Claude Code runs the entire chain on this one approval; Codex chains the queue but stops each child at its own native plan approval, which only the operator can give — a recorded degradation in the parity ledger ADR-0097 requires, never a local variant.
4. **The chain** (ADR-0138) — a child is armed with the intent phase 1 settled, its own decisions, the global entries whose scope covers it, the policy, and the fact that it IS a child; it then runs Mode 1 start to finish and ends on one of exactly two words this chain reads, CLOSED or SET ASIDE (ADR-0137), and the next child arms on either. Before EVERY arming the tree it hands over is read: `git status --porcelain` EMPTY in the main checkout, and the worktree root beside it, because a child set aside mid-wave dirties the checkout nowhere while its standing worktrees hold the very paths the next child's own first cut collides on. A bar that fails arms nothing further — every un-run child is set aside with the reason, and nothing the bar stopped on is discarded, committed under either child's name, or armed over anyway. Position and disposition are durable: the ledger topic, the index row's `NEXT:` line, and a `roadmap` runtime key that gates nothing, armed and disarmed by the flow, dropped by the SessionEnd teardown and named by the SessionStart signal so a session that died leaves the next one its resume route.
5. **The presence phase** (ADR-0139) — the flow's one return to the operator after phase 3. Four classes reach it and the list is closed: every decision phase 2 queued, carrying the options the silent tier would have put to them; every child a queued decision set aside, as ONE item with that decision; whatever stopped the chain, with the path of any standing worktree only the operator can dispose of; and every child's own `PENDING:` rows and named residuals, from the children that closed as much as from the ones set aside. `Prioritized` means ORDERED by what an answer releases — what stopped the chain, what set a child aside, what cost its child nothing, what merely awaits their hand — tie-broken by phase 1's run order and then by the point inside the child, so the same run consolidated twice yields the same queue. One final report heads it: what was decided for them and on what rationale, what was deferred and why, what awaited their hand. Three moments may interrupt an absent operator — the roadmap COMPLETE, the queue READY, the chain BLOCKED — and a per-child close is deliberately none of them.
