...
**2026-07-25 — harness-hardening pass (this change)**
...
5. **Close** — `oso-code:quality-pass` on the touched code, with `oso-code:debt-sweep` offered beside it — additive, never instead of it — only when the fix sprawled across many files, and the quality pass re-run over whatever cleanup that sweep lands; a pre-commit `oso-code:security-pass` offer if it touched data models, auth, or payments; session summary to engram; commit/push/PR only if the user asks.
