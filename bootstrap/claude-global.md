...
- Content the operator must read ends the turn as plain text — never a tool call in the same turn (the TUI drops it). The one exception is armed, never assumed: while this repository's state carries `auto=running`, the run is unattended and its milestones ride the stream instead of ending the turn — each one also appended with `oso-state journal` — while the park and the final report still end it.
