...
initialize_paths() {
  ...
  RESTORE_EXERCISED_MARKER="$BACKUPS_ROOT/.install-restore-verified-opencode"
}
...
list_backups() {
  ...
  listing="$(mktemp "${TMPDIR:-/tmp}/oso-opencode-restore-list.XXXXXX")"
  opencode_install_backups_newest_first "$BACKUPS_ROOT" > "$listing"
  if [ ! -s "$listing" ]; then
    info "no install-opencode.sh backups found under $BACKUPS_ROOT"
    rm -f "$listing"
    return 0
  fi
...
resolve_backup_dir() {
  ...
  listing="$(mktemp "${TMPDIR:-/tmp}/oso-opencode-restore-latest.XXXXXX")"
  opencode_install_backups_newest_first "$BACKUPS_ROOT" > "$listing"
  RESTORE_TARGET="$(awk 'NR == 1 { print; exit }' "$listing")"
  rm -f "$listing"
  [ -n "$RESTORE_TARGET" ] ||
    fail "no install-opencode.sh backups found under $BACKUPS_ROOT"
}
...
validate_backup_dir() {
  ...
  is_opencode_install_backup "$dir" ||
    fail "not an install-opencode.sh backup: $dir — restore-codex.sh replays the Codex installer's own snapshots"
...
confirm_restore() {
  ...
  info "this overwrites the current OpenCode install state with the snapshot at $RESTORE_TARGET"
...
main() {
  ...
  if restore_backup_manifest "$RESTORE_TARGET/manifest" "$RESTORE_TARGET/items"; then
    ...
    warn "core.hooksPath (the git commit-hook wiring) is not covered by this restore; re-run install-opencode.sh if that also needs to change"
  else
