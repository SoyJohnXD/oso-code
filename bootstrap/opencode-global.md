...
- Substantial changes (features, refactors, anything needing architecture or contract decisions): the operator starts `/oso-plan`.
- Small, quickly verifiable changes: the operator starts `/oso-quick`.
- Something broke (a bug, a crash, a failing behavior): the operator starts `/oso-debug`.
- Several substantial changes known up front, decided in one sitting and run one after another: the operator starts `/oso-roadmap`; one approval covers the whole queue and the chain then runs every child unattended.
- Every change closes at zero warnings. `/oso-plan` commits each slice as it lands; never push or open a PR unless asked. The one exception is armed, never assumed: a run under the AUTO disposition finishes by pushing its own change branch and opening the PR for it, and stops there — the merge, a release and any production deploy stay yours.
