...
2. `../_shared/platform/opencode/roadmap.md` — what the flow leaves to the host: the agent every phase runs on, how far the chain runs unattended, the approval gate its one approval crosses, the paths it resolves, and the state command every runtime write of the chain runs under, which that file routes to `plan.md` beside it rather than spelling twice.
