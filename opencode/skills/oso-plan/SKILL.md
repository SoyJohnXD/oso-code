...
2. **Apply (subagent)** — launch the `oso-applier` agent with the slice (goal, files, verify criteria), the DECISION BLOCK, the project conventions, the rubric path (`_shared/rubric.md`), and the two coordinates that place the work — the WORKTREE PATH (the main checkout under SEQUENTIAL) and SLICE START. The applier runs on the model the profile names for its role, named in the Launching milestone. The DECISION BLOCK is every ledger decision relevant to this slice, copied from the ledger record BY ID — verbatim and whole, never paraphrased and never quoted in part. The slice's Verify line travels as §4 wrote it: its failing check is what the `red:` and `green:` lines exercise, and a `Verify-exception` written there instead is what `red: exception — <that line's reason>` carries in its place.
...
3. **Verify (subagent)** — launch the `oso-verifier` agent with the slice criteria, the zero-warnings commands from the ledger, the rubric path, step 2's DECISION BLOCK unchanged, `applier_proof`, and the same two coordinates, diffing `HEAD` since step 2 — this slice's own pending work alone, never a sibling slice already committed beside it. The verifier runs on the model the profile names for its role, named in the Launching milestone. `applier_proof` carries exactly the applier's `proof:`, `scan:`, and `decisions_used:` blocks, verbatim: never its narrative or other report fields, since a judge who reads the author's story first agrees with it.
   - On `fail`: relaunch the applier on the same slice assignment, carrying the verifier's findings VERBATIM — every one with its `file:line` and evidence. A refuted claim among the verdict's `claims:` lines is one of those findings — a criterion of this slice, self-reported and not observed — and rides that same relaunch. Loop apply → verify until it passes. A finding grounded in one of §2 step 6's four no-exception rules is never yours to overrule: FIX (the relaunch above) or ESCALATE it to the operator with options and tradeoffs. No payload you build may instruct any judge away from one of those rules — a standing ruling written into the next verifier's payload is that same overrule with a longer reach. Under a ROADMAP, ESCALATE is the reconciliation-class question the ground rules already queue, and the change is set aside on it. §3's doubt-pass reconciliation, and §7 step 3's bare-tag rule, stand outside this.
