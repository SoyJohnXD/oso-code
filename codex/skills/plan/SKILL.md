...
2. `../_shared/platform/codex/plan.md` — what the flow leaves to the host: the tools it calls, the paths it resolves, the approval gate, the state command, and the parts of this port a later slice still has to fill.

Where the neutral body defers to "your host", the platform file is the answer, and it is the only answer. A section of it marked PLACEHOLDER has no answer yet: say so to the operator and take the fallback it names — never improvise a spelling in its place.
