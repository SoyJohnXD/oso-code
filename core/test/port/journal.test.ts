...
for (const subject of STATE_SUBJECTS) {
  describe(
    `${subject.name}: the journal timestamp format, which journal_texts_in in tests/hooks-test.sh discards before comparing — port tests read from plugin/bin/oso-state:488, never parity evidence`,
    { skip: skipUnlessSpawnable(subject) },
    () => {
      test("journal stamps its milestone with a UTC date -u +%Y-%m-%dT%H:%M:%SZ timestamp (read from plugin/bin/oso-state:488)", () => {
