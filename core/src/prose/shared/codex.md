...
Next run `oso-state handoff wait` with the exact slice, attempt, resolved UUID as `--agent-id`, agent type and `--timeout 10`, then `oso-state handoff consume` with those same four identity fields. Only successful wait followed by one-shot consume satisfies the FILE PRECONDITION for interpreting this final report. These commands use the current repository as their outer identity and need no `--session`. A timeout, mismatch, stale or replayed receipt blocks the flow; malformed reports block without a format-repair followup.
