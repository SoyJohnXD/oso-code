...
It ends on exactly one of: `Doubt Pass: clean` — report it and the freeze proceeds; `Doubt Pass: findings` — report each finding verbatim for reconciliation; `Doubt Pass: blocked` — resolve what the payload names missing and invoke the judge again fresh.
