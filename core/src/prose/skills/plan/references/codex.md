...
This host publishes the fixed marker `OSO_AGENT=1` through `shell_environment_policy.set` for tool subprocesses and as an explicit environment prefix on every installed user-hook command. Spell repository-runtime state calls as `"${OSO_STATE_BIN:-oso-state}" --session "${OSO_AGENT}" <verb> …`. Runtime state is intentionally shared by repository, so this marker is audit metadata and the common value the `session` key — the OWNERSHIP identity — carries for model-issued state writes, the edit/commit readers, teardown and the git hook; never a claim that Codex supplied a hidden session id. For session-bound amend-plan, use the actual owning native CODEX_THREAD_ID, never OSO_AGENT. A handoff receipt's own `hook_session` field carries the same fixed marker as every other hook-issued value, never a real identity; what a handoff correlates against instead is the native rollout, and each verb proves a different part of it. `resolve-codex` is the one bound to the caller itself — it searches only rollouts whose recorded parent IS the current CODEX_THREAD_ID, in this repository, at the asserted agent path and role, and returns the single Codex-assigned agent id that answers. `adopt` and `consume` prove an agent id the caller already holds: sole matching rollout, same repository, that exact agent path and agent role, a live unconsumed receipt for this slice and attempt, and a parent — any parent, never necessarily the caller, the one clause this pair deliberately drops so a resumed chat, issued a new thread id, can still claim the delegation it parked. `wait` proves nothing beyond the coordinates it was handed. Read all three as PROVENANCE — what stops a cooperating agent from claiming a receipt that is not its own — and never as an authorization boundary: receipts and rollouts alike resolve through same-uid filesystem paths and environment variables, so a hostile process running as you is answered by OS-level separation, which this rail neither provides nor attempts. Plan approval keeps a second, separate key because the shared marker names no session there either: `plan_approval_session` is written only by `capture-plan` under the real session Stop observed, and `approve-plan`, `cancel-plan` and `amend-plan` compare against it rather than `session`, so a later `session`-overwriting write under the shared marker can never unbind a still-pending approval from the session that presented it.
