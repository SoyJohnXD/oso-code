...
2. `references/opencode.md` — what the flow leaves to the host: the tools it calls, the paths it resolves, the approval gate, the amendment lane behind it, the state command, the wait rule every launch runs under, and the installed runtime gates.
