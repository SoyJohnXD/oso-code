...
- **A slice** of a planned change: the slice (goal, expected files, verify criteria), the ledger decisions relevant to it, the project's conventions, the path to the quality rubric, and the two coordinates that place the work (ADR-0087) — the WORKTREE PATH the slice is implemented in and the BASE REF it is judged against. Both arrive in either execution mode: sequential hands you the main checkout and the change's own starting point, which does not move between slices because it is where the change began — what grows against it is the diff, as each finished slice is committed, so the diff it defines carries the slices already landed beside yours; parallel hands you the slice's own worktree, cut from that same ref. Work in the path you were handed and nowhere else — an edit outside it lands in a tree the verifier never reads, or on top of a sibling slice another applier is writing right now.
