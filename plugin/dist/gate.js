...
function hookIdentityField(payload, caller, field) {
  ...
  const value = parsed.document[field];
  return typeof value === "string" ? value : "";
}
function jsonCommandLine(payload, caller) {
...
import { mkdirSync as mkdirSync2, rmSync as rmSync2, statSync as statSync2, writeFileSync as writeFileSync2 } from "node:fs";
...
function delegationNamedBy(envelope, markerLineCount) {
  const named2 = MARKER.exec(envelope.lastAssistantMessage.split("\n")[0] ?? "");
  if (markerLineCount !== 1 || named2 === null || envelope.agentId === "") return void 0;
  return { slice: named2[1], attempt: named2[2], agentId: envelope.agentId };
}
...
function judgePlanprompt({ envelope }) {
  ...
  const named2 = stateFileIfNamed(envelope.cwd);
  const reachable = controlPromptReaches(envelope, sessionId, action, named2);
  ...
  if (named2 === void 0 || !isReadableRegularFile(named2)) return control(NO_PENDING_PLAN);
  const stateFile = named2;
