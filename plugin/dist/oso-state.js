...
var NUL_BYTE = 0;
var EMPTY_GIT_CONFIG = "/dev/null";
var GIT_FILTER_CONFIG_PATTERN = "^filter\\..*\\.(clean|process)$";
var GIT_FILTER_CONFIG_KEY = /^filter\.(.+)\.(?:clean|process)$/;
...
function renderScan(hits, coverage) {
  const cited = hits.map(({ file, line, note, category }) => {
...
`;
  ...
  const candidateEvidence = candidates === 0 ? "" : `; ${candidates} registered generated-output candidate${candidates === 1 ? "" : "s"} require exact regeneration evidence`;
...
function generatedBundleClause(generatedBundles) {
  if (generatedBundles.length === 0) return "";
  return `; ${generatedBundles.length} generated bundle(s) were not counted: ${generatedBundles.map(({ file }) => file).join(", ")}`;
}
...
function inlineCommentsAddedIn({ file, text, addedLines, language }) {
  const category = GENERATED_BUNDLES.includes(file) ? "registered-generated-output-candidate" : void 0;
...
var StateRootUnwritableError = class extends Error {
  ...
  constructor(directory, cause) {
    super(
      `cannot write the oso-code state directory ${directory}: ${cause}. The gates read what it holds and treat an unwritten state as no armed session, so arming here would leave them unable to see their own state. Grant the directory to the active permission mode \u2014 add it to that profile's workspace roots, launch with --add-dir ${directory}, or pick a mode that can write \u2014 then arm again.`
    );
...
function requireWritableStateRoot() {
  ...
  try {
    ...
    throw new StateRootUnwritableError(directory, causeOf2(error));
  }
}
...
function withLock(stateFile, sessionId, run, acquisition = "retry") {
...
var UNPUBLISHED_KEYS = ["version", "slice", "attempt", "reason"];
...
function runHandoffResolveCodex(cwd, coordinates) {
  ...
  try {
    ...
    throw asNativeCodexFailure("resolve", error);
  }
}
...
function runHandoffPublish(cwd, coordinates, hookSession) {
  ...
  try {
    ...
    rmSync2(paths.unpublished, { force: true });
  } finally {
...
function runHandoffConsume(cwd, claim) {
  ...
  try {
    ...
    if (!existsSync2(paths.receipt)) {
      throw new HandoffFailure(`no unconsumed receipt for slice ${claim.slice} attempt ${claim.attempt}`);
    }
    requireMatchingReceipt(paths.receipt, claim, "receipt identity does not match the delegated result");
    const content = readFileSync3(paths.receipt, "utf8");
    writeWatermark(paths, claim.attempt);
...
function matchingReceiptOrStop(paths, coordinates) {
  ...
  const refusal2 = unpublishedRefusalOf(paths, coordinates);
  if (refusal2 !== void 0) {
    throw new HandoffFailure(
      `agent ${coordinates.agentId} finished slice ${coordinates.slice} attempt ${coordinates.attempt}, but its SubagentStop could not publish the receipt: ${refusal2}. Read that agent's own final message for the verdict it produced; relaunching it would repeat work it has already done.`
    );
  }
  return void 0;
}
function unpublishedRefusalOf(paths, delegation) {
...
function requireCanonicalAgentPath(agentPath) {
  if (!CANONICAL_CODEX_AGENT_PATH_PATTERN.test(agentPath)) throw new HandoffFailure("invalid canonical Codex agent path");
}
...
function handoffPaths(cwd, agentId) {
  ...
  return {
    ...
    unpublished: path4.join(directory, `${agentKey}.unpublished`),
...
function asNativeCodexFailure(verb, error) {
  if (error instanceof HandoffFailure) return error;
...
function confirmCodexDelegation(cwd, claim, deadline) {
  ...
  if (nativeRepositoryIdentity(metadata.cwd, deadline) !== repository) {
    throw new HandoffFailure("the asserted agent's native rollout belongs to another repository");
  }
  if (metadata.parentThreadId === void 0) throw new HandoffFailure("the asserted agent's native rollout has no parent");
  if (metadata.agentPath !== claim.agentPath) throw new HandoffFailure("the asserted agent path does not match its native rollout");
  if (metadata.agentRole !== claim.agentType) throw new HandoffFailure("the asserted agent role does not match its native rollout");
...
function requireLiveCodexReceipt(cwd, claim, deadline) {
  const directory = receiptDirectoryFor(cwd);
  const name = `${sha256Hex(claim.agentId)}.receipt`;
  if (!existsSync2(path4.join(directory, name)) || readCurrentCodexReceipt(directory, name, claim, deadline) === void 0) {
    throw new HandoffFailure(`no live receipt for slice ${claim.slice} attempt ${claim.attempt}`);
  }
}
...
var PlanApprovalError = class extends PlanFailure {
};
function matchesPlanPresentation(stateFile, binding) {
  return readValue(stateFile, "plan_presentation_version") === String(binding.version) && readValue(stateFile, "plan_presentation_turn") === binding.turnId && readValue(stateFile, "plan_presentation_message") === binding.messageId && readValue(stateFile, "plan_presentation_content_digest") === binding.contentDigest;
}
var PlanVerifyFailure = class extends PlanFailure {
  slice;
  constructor(slice) {
...
function runRejectPlanPresentation(cwd, sessionId, fingerprint) {
  ...
  withLock(stateFile, sessionId, () => {
    writeStatePairs(stateFile, [
      ...
      "plan_presentation_status=failed",
      `plan_approval_digest=${readValue(stateFile, "plan_approval_digest") ?? fingerprint}`
    ], sessionId);
  });
}
function readPlanForReplacement(cwd, sessionId) {
  const stateFile = stateFileFor(cwd);
  return withLock(stateFile, sessionId, () => {
    if (readValue(stateFile, "plan_approval_session") !== sessionId || readValue(stateFile, "plan_approval") !== "pending") throw new PlanFailure("replacement requires an own pending plan", { code: "replacement-not-pending" });
  ...
  return withLock(stateFile, sessionId, () => {
    ...
    writeStatePairs(
      ...
      [
        ...
        "plan_revision=0",
        ...binding === void 0 && readValue(stateFile, "plan_presentation_version") === void 0 ? [] : [
          ...
          `plan_presentation_status=${binding === void 0 ? "" : "captured"}`
        ]
      ],
      sessionId
    );
...
function runApprovePlan(cwd, sessionId, digest, nativePresentation) {
  ...
  return withLock(stateFile, sessionId, () => {
    ...
    let native;
    if (nativePresentation !== void 0) {
      if (readValue(stateFile, "plan_presentation_status") !== "captured" || readValue(stateFile, "plan_presentation_version") !== "1") throw new PlanApprovalError("native presentation requires replacement capture", { code: "replacement-required" });
...
function recordTheFinish({ envelope, delegation, reason }) {
  ...
  try {
    ...
    return `oso-code: SubagentStop could not record that its child finished either: ${causeOf2(cause)}
`;
  }
}
...
// core/src/hosts/codex-presentation.ts
var PLAN_MARKER = "<!-- oso-plan-approval: v=2 action=IMPLEMENT_THE_PLAN -->";
var PLAN_MARKER_PREFIX = "<!-- oso-plan-approval:";
var MAX_TRANSCRIPT_BYTES = 64 * 1024 * 1024;
var CodexPresentationFailure = class extends PlanFailure {
  code;
...
function readCodexTranscript(envelope) {
  try {
    const deadline = performance.now() + CODEX_METADATA_READINESS_MS;
...
function presentationSegment(records, currentTurn, precedingApproval) {
  ...
  if (precedingApproval) {
    ...
    const presentations = tasks.filter((index, position) => {
      ...
      return records[index]?.payload["collaboration_mode_kind"] === "plan" || records.slice(index + 1, taskEnd).some((record) => isPlan(record) || isFinal(record) && textContent(record.payload["item"]["content"], "Text").includes(PLAN_MARKER_PREFIX));
    });
    start = presentations.at(-1) ?? -1;
    end = tasks.find((index) => index > start) ?? boundary;
  }
  const task = records[start];
  if (task === void 0 || task.payload["collaboration_mode_kind"] !== "plan") throw new CodexPresentationFailure("not-plan-presentation");
...
function finalPair(segment, sessionId, turnId) {
  if (segment.some((record) => record.type === "event_msg" && isRecord(record.payload["item"]) && record.payload["item"]["type"] === "AgentMessage" && record.payload["item"]["phase"] === void 0)) throw new CodexPresentationFailure("incomplete-final");
  const finals = segment.flatMap((record, index) => isFinal(record) ? [index] : []);
  const finalIndex = finals.at(-1);
  if (finalIndex === void 0) throw new CodexPresentationFailure("missing-final");
  const previousFinal = finals.at(-2) ?? -1;
  const final = segment[finalIndex];
  const item = final.payload["item"];
  const messageId = item["id"];
  if (typeof messageId !== "string" || !isNameToken(messageId) || finals.some((index) => index !== finalIndex && (segment[index]?.payload["item"])["id"] === messageId)) throw new CodexPresentationFailure("ambiguous-final");
...
function pairedDocument(input) {
  ...
  const planItem = plan.payload["item"];
...
function requireOwner(record, sessionId, turnId) {
...
function resolveCodexTurn(envelope) {
  if (envelope.caller.host === "codex" && envelope.transcriptPath !== "") {
    ...
    const candidates = tasks.filter((task2) => task2.payload["turn_id"] === envelope.turnId);
    const task = candidates[0];
    if (candidates.length !== 1 || task !== tasks.at(-1)) return UNATTESTED;
    const kind = task?.payload["collaboration_mode_kind"];
    const mode = typeof kind === "string" ? ATTESTABLE_MODES[kind] : void 0;
    return mode === void 0 ? UNATTESTED : { mode, source: "transcript" };
  }
...
var MAX_DIAGNOSTIC_SLICE_LENGTH = 80;
...
function judgePlanstop({ envelope }) {
  ...
  if (!lastLineOf(message).startsWith(PLAN_MARKER_PREFIX)) return SILENT2;
...
function planItemDocument(envelope, turn, rawMessage) {
  ...
  if (planDocument.split("\n").some((line) => line.startsWith(PLAN_MARKER_PREFIX))) return SELF_MARKING_PLAN_ITEM;
...
function markerIsTheWholeEnding(message, rawMessage) {
  ...
  return rawEndsWithMarker && lines.filter((line) => line.startsWith(PLAN_MARKER_PREFIX)).length === 1 && lines.filter((line) => line === PLAN_MARKER).length === 1;
}
...
var HANDOFF_SUBACTIONS = ["publish", "wait", "consume", "resolve-codex", "adopt"];
var HANDOFF_FLAGS = {
  "--slice": "slice",
  "--attempt": "attempt",
  "--agent-id": "agentId",
...
function dispatchHandoff(remaining) {
  ...
  switch (subaction) {
    case "resolve-codex":
      process.stdout.write(runHandoffResolveCodex(cwd, claim) + "\n");
      return 0;
    case "adopt":
      runHandoffAdopt(cwd, claim);
      return 0;
    case "publish":
      readStdin();
    ...
    case "wait":
      ...
      return 0;
    case "consume":
      process.stdout.write(runHandoffConsume(cwd, claim));
      return 0;
  }
}
...
function checkHandoffCoordinateShape(subaction, coordinates) {
  const hasAgentPath = coordinates.agentPath !== void 0;
  const hasTimeout = coordinates.timeout !== void 0;
  const hasHookSession = coordinates.hookSession !== void 0;
  if (subaction === "resolve-codex" && (!hasAgentPath || coordinates.agentId !== void 0 || hasTimeout || hasHookSession)) {
    throw new UsageError();
  }
