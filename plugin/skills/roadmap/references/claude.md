...
Every milestone report the chain and its children make while the operator is away is the exception, and `${CLAUDE_SKILL_DIR}/../_shared/references/claude.md`'s **The unattended run — the carve-out, and the record that pays for it** section states it whole rather than this section restating it: a chain arming its children under `auto=running` is an UNATTENDED RUN, so its milestone text rides the stream instead of ending the turn and is journaled full-text in its place, while §5's three stopping moments — the roadmap complete, the queue ready, the chain blocked — end the turn the way this section opens, because each is the chain handing itself back.
