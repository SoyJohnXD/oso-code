...
2. `${CLAUDE_SKILL_DIR}/references/claude.md` — what the flow leaves to the host: how far the chain runs unattended, the paths it interpolates, the delivery rule this TUI imposes, and the state command every runtime write of the chain runs under, which that file routes to the plan skill's own reference rather than spelling twice.
