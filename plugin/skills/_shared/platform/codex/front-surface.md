...
- When Impeccable is absent, ask the operator to rerun `bootstrap/install-codex.sh`, which installs and remounts it. Continue without the design bar and record the gap where the invoking mode requires; never repeat Claude's `/plugin` commands and never use a discovered cache path as a temporary mount.
