...
**The named residual**, which excepts the count rule of the judge's-outcome milestone as well. A judge's loop is allowed to end with its lowest tier of findings still open, and the mode that ran it NAMES what it left rather than dropping it: the debt sweep's `nit` residual (`plan/SKILL.md` §7, `bodies/debug.md` §5) and the design audit's P2/P3 residual (`_shared/front-surface.md`) each land in that mode's own record. Such a residual also reaches the operator VERBATIM at the close — every finding as the judge wrote it, never a count and never a paraphrase — and that list alone is exempt from the bound above. It excepts those two rules and no others: the same loop's rounds are still reported as counts while it runs, and every other milestone keeps its three lines.
