...
**A roadmap's final report**, which excepts the bound alone. The ROADMAP mode's §5 ends a run the operator was absent for with one report over the whole queue of changes — what was decided for them and on what rationale, what was deferred and why, and what awaited their hand — and three lines cannot carry an account of a run nobody watched. So that report is exempt, stated here as the exception it is rather than left to an agent inferring an exception from the fact that no bound could fit one. Nothing else in that run is: every milestone above keeps its three lines while the chain runs, a child's disposition included, and the ordered queue the report heads was never a milestone report at all — it is operator-facing content the way a plan document is, which this contract has never bounded.
