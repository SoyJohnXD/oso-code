...
Deliver full Engram decisions and all three `applier_proof` blocks in band; create no auxiliary per-slice copies in any format. Preserve journals, native records, runtime-owned approval snapshots, `current.md`, state, receipts and history. Reuse history only after reconciling corresponding decisions and execution; filenames and dates grant no authority. Apply the verifier's **Evidence and acceptance** contract; host bindings govern identity and delivery.

A milestone report is operator-facing content: its delivery is host-bound, and the reference file for that host states the rule. What a host's own UI does or does not surface alongside a launch is a fact about that host, never about this contract, and it is bound once per host under `_shared/references/claude.md`, `_shared/references/codex.md` and `_shared/references/opencode.md`.

A host's reference file MAY also carve an UNATTENDED RUN out of its delivery rule — a run whose routine milestones ride the stream instead of ending the turn. Where a host does, its own file states the carve-out whole: what marks a run unattended, what the stream then costs, and which deliveries still end the turn — the park and the final report, at minimum, since each hands the run back rather than reporting it. What replaces the interrupted stream is the same thing on every host that takes the carve-out: each milestone is ALSO appended full-text to the run's own journal (`oso-state journal`), the durable record an operator reads on their return and the only one a compaction cannot take. Where a host permits neither routine same-turn delivery nor an unattended carve-out, every milestone above ends the turn as plain text, with no tool call after it in that turn.
