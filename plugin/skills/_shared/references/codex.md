...
Next run `oso-state handoff wait` with the exact slice, attempt, resolved UUID as `--agent-id`, agent type and `--timeout 10`, then `oso-state handoff consume` with those same four identity fields plus the retained canonical `--agent-path`. Only successful wait followed by one-shot consume satisfies the FILE PRECONDITION for interpreting this final report. These commands use the current repository as their outer identity and need no `--session`. A timeout, mismatch, stale or replayed receipt blocks the flow; evaluate the delivered verifier report under its shared **Evidence and acceptance** contract.

When the caller resuming this flow is not the original parent — its own `CODEX_THREAD_ID` no longer matches the child's recorded parent — recover the retained UUID and canonical path from the checkpoint and run `oso-state handoff adopt --agent-id <uuid> --agent-path <canonical> --slice <id> --attempt <n> --agent-type <role>` in place of `resolve-codex`. Adopt proves that exact asserted identity against the native rollout rather than searching for it, then wait and consume proceed unchanged.
...
A slice is ready for verification when its own handshake is consumed and its owned handles are reconciled under **Completion handshake** and **Correction and quiescence** above. `../parallel.md`'s own readiness paragraph is the one place that answers whether a slice's verification may open early while the rest of its wave is still in flight, never a policy this section states on its own. A ready, isolated and quiescent slice may be verified while unrelated wave work is still in flight, inside the wave's existing dependency and isolation barriers and never in place of them, but only where that paragraph opts a host in by name. No reference file currently takes that route, so every Codex wave keeps its unconditional default instead: every applier first, then the verifications. Codex owns its own subagent threads and this harness rations none of them: it reserves no slot, certifies no released capacity, and a spawn this host refuses is an ordinary tool-call error to report, never a park, a stop or a reduced check.
