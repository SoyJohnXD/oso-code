...
The three operational roles carry their complete contract in their agent definition. The model a launch runs on is pinned there too: this host has no per-launch model parameter, so the model clause the neutral body carries resolves to the role's own pinned `model`, which is what the profile records as `codex: pinned by host contract` and what the Launching milestone names. Give them the payload the neutral body names. Because a Codex role cannot set its working directory, every applier and verifier payload includes the absolute WORKTREE PATH and the ref coordinate the body names for that launch — SLICE START or WAVE START — and every integrator payload also includes the absolute main-checkout path beside the wave's branches and worktrees.
