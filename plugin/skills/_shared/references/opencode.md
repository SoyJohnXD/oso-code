...
Routine informational milestones may continue in the same turn with the tool call that advances the flow. Questions, permission requests, operator-dependent blockers and completion remain turn boundaries. Native approvals remain genuine operator grants and exact-document gates.

## The unattended run — routine continuity and the record that carries it

An UNATTENDED RUN is this repository's runtime state carrying `auto=running` for this session: the marker the AUTO disposition arms (`plan/SKILL.md`'s ground rules) and the ROADMAP chain arms per child (`roadmap/SKILL.md` §4). This host's existing continuation rail handles any later turn by posting the next turn itself: the installed plugin answers `session.idle` with a real user turn back into the same session, where Claude Code has only a `Stop` decision to refuse one with. The rail does not replace a required operator answer or approval.
