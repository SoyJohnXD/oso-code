...
1. Close via the quality-pass judge on the touched code; ONLY when the fix sprawled across many files, offer the debt-sweep judge as well — ADDITIVE to that pass, never instead of it, because step 4's green reads `Quality Pass: passed` and a branch that traded the pass for the sweep could never reach green at all. When the fix touched front surface, debug never runs Impeccable's `init` or `document` — a bug fix does not bootstrap a design system; if the project has no `DESIGN.md`/`PRODUCT.md`, the pinned detect (§4) still ran as the design gate and the missing-design-docs gap is named here in one clause.
   ...
   - `Debt Sweep: findings` → the list goes to the `oso-applier` agent as a debt cleanup assignment: the smallest edit that FULLY resolves each finding — behavior-preserving; structural findings may span files. Then re-invoke the debt-sweep judge on the same arguments to confirm, and loop judge → fix until `Debt Sweep: clean`.
