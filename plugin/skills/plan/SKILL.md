...
2. **Apply (subagent)** — launch the `oso-applier` agent with the slice (goal, files, verify criteria), every ledger decision relevant to it, the project conventions, the rubric path (`_shared/rubric.md`), and the two coordinates that place the work — the WORKTREE PATH (the main checkout under SEQUENTIAL) and SLICE START. The applier runs on the model the profile names for its role, named in the Launching milestone.
