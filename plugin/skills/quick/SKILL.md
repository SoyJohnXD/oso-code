...
- **Absence policy (ADR-0046)** — if Impeccable is not installed, follow the absence policy in `${CLAUDE_SKILL_DIR}/../_shared/front-surface.md`; quick records the gap in the close's session summary.
...
- Stop-the-line (ADR-0043) — breakage unrelated to the change discovered while iterating is never fixed in passing: name it and offer `oso-code:debug`; declining is noted in the close's session summary and iteration continues.
