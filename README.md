...
**Surfaces**: oso-code is installed and verified against the local Claude Code and Codex runtimes. Codex's native approval UI submits `Implement the plan.`; Oso's guarantee composes that interaction with its local `Stop` → `UserPromptSubmit` → `PreToolUse` digest path. The common prompt is intercepted only for a same-session pending Oso document. Hosted or specialized execution surfaces, and `write_stdin` calls against an already-running process, do not cross that complete rail.
